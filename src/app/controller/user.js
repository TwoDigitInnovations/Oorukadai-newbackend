...
const { sendMailWithSubjectViaSendgrid, sendMailWithSTemplateViaSendgrid } = require("../services/sendgrid");
...
module.exports = {
  ...
  sendMessageToCustomer: async (req, res) => {
    try {
      const { email, message, templateId } = req.body;

      if (templateId) {
        await sendMailWithSTemplateViaSendgrid(email, templateId)
      }
  ...
  sendMessageToAllCustomer: async (req, res) => {
    try {
      ...
      if (!message.trim() && !templateId.trim()) {
        return response.error(res, { message: "Message or Templat ID is required" });
      }

      const users = await User.find({ type: "USER" }).select("email name");
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        try {
          group = result[i];
          if (templateId) {
            await sendMailWithSTemplateViaSendgrid(group, templateId)
          }
          if (message) {
